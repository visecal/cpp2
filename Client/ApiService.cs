...
namespace subphimv1.Services
{
    ...
    {
        // Server URLs - chỉ tồn tại trong code, không lưu ra registry
        private const string Server1Url = "http://34.173.37.168/";
        private const string Server2Url = "http://34.173.37.168/";

        /// <summary>
        /// Map chỉ số server với URL thực tế. URL chỉ tồn tại trong code, không lưu ra registry.
        /// </summary>
        public static string GetServerUrlByIndex(int index)
        {
            return index == 1 ? Server2Url : Server1Url;
        }
        ...
        static ApiService()
        {
            // Load server index đã lưu từ registry và map với URL
            int serverIndex = CredentialService.LoadServerIndex();
            _apiBaseUrl = GetServerUrlByIndex(serverIndex);
