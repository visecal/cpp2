...
namespace SubPhim.Server.Services
{
    ...
    public class SubtitleOrchestratorService : ISubtitleOrchestratorService
    {
            ...
            List<SubtitleApiKey> apiKeys)
        {
            ...
            foreach (var batch in batchPlan)
            {
                ...
                for (int i = 0; i < settings.ApiKeysPerServer && keyIndex < apiKeys.Count * 2; i++)
                {
                    ...
                    // Skip if in memory cooldown
                    if (_apiKeyCooldowns.TryGetValue(key.Id, out var cooldownUntil) && cooldownUntil > DateTime.UtcNow)
                        continue;

                    try
                    {
                        var decryptedKey = _encryptionService.Decrypt(key.EncryptedApiKey, key.Iv);
                        keysForServer.Add(decryptedKey);
                    }
                    catch
                    {
                        _logger.LogWarning("Failed to decrypt API key {KeyId}", key.Id);
                    }
                }

                if (!keysForServer.Any())
                {
                    _logger.LogError("No API keys available for batch {BatchIndex}", batch.BatchIndex);
                    continue;
                }
            ...
            SemaphoreSlim semaphore)
        {
            ...
            try
            {
                ...
                var serverRequest = new
                {
                    ...
                    callbackUrl = settings.EnableCallback ? GetCallbackUrl(job.SessionId, task.BatchIndex) : null
                };
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        _logger.LogInformation("Batch {BatchIndex} sent to server {ServerId}", task.BatchIndex, server.Id);
                    }
                    else
                    {
                        var error = await response.Content.ReadAsStringAsync();
                        taskInDb.Status = ServerTaskStatus.Failed;
                        taskInDb.ErrorMessage = $"HTTP {(int)response.StatusCode}: {error}";
                        serverInDb.FailureCount++;
                        serverInDb.LastFailedAt = DateTime.UtcNow;
                        serverInDb.LastFailureReason = taskInDb.ErrorMessage;
                        _logger.LogError("Batch {BatchIndex} failed: {Error}", task.BatchIndex, taskInDb.ErrorMessage);
                    }
                }
                catch (Exception ex)
                {
                    taskInDb.Status = ServerTaskStatus.Failed;
                    taskInDb.ErrorMessage = ex.Message;
                    serverInDb.FailureCount++;
                    serverInDb.LastFailedAt = DateTime.UtcNow;
                    serverInDb.LastFailureReason = ex.Message;
                    _logger.LogError(ex, "Error sending batch {BatchIndex} to server {ServerId}", task.BatchIndex, server.Id);
                }
        ...
        private string GetCallbackUrl(string sessionId, int batchIndex)
        {
            // This should be configured based on your server's public URL
            return $"/api/subtitle/callback/{sessionId}/{batchIndex}";
        }
        ...
        public async Task ProcessCallbackAsync(ServerCallbackData callback)
        {
            ...
            // Parse session ID to get original job
            var parts = callback.SessionId.Split("_batch");
            if (parts.Length != 2) return;

            var originalSessionId = parts[0];
            if (!int.TryParse(parts[1], out var batchIndex)) return;
